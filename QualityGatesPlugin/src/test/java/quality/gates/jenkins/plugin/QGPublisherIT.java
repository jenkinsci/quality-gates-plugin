...
public class QGPublisherIT {
    ...
    public void testPerformShouldFailBecauseOfPreviousSteps() throws Exception {
        ...
        doReturn(null).when(buildDecision).chooseSonarInstance(globalConfig, jobConfigData);
        doReturn(Result.FAILURE).when(buildDecision).getStatus(globalConfigDataForSonarInstance, jobConfigData);
        jenkinsRule.assertBuildStatus(Result.FAILURE, buildProject(freeStyleProject));
        Run lastRun = freeStyleProject._getRuns().newestValue();
    ...
    public void testPerformShouldSucceedWithNoWarning() throws Exception {
        ...
        when(buildDecision.getStatus(any(GlobalConfigDataForSonarInstance.class),any(JobConfigData.class))).thenReturn(Result.SUCCESS, Result.SUCCESS);
    ...
    public void testPerformShouldSucceedUnstableWithNoWarning() throws Exception {
        setGlobalConfigDataAndJobConfigDataNames(TEST_NAME, TEST_NAME);
        jobConfigData.setProjectKey("projectKey");
        jobConfigData.setIgnoreWarnings(true);
        doReturn(globalConfigDataForSonarInstance).when(buildDecision).chooseSonarInstance(globalConfig, jobConfigData);
        when(buildDecision.getStatus(any(GlobalConfigDataForSonarInstance.class),any(JobConfigData.class))).thenReturn(Result.SUCCESS, Result.UNSTABLE);
    ...
    public void testPerformShouldFail() throws Exception {
        setGlobalConfigDataAndJobConfigDataNames(TEST_NAME, TEST_NAME);
        doReturn(globalConfigDataForSonarInstance).when(buildDecision).chooseSonarInstance(globalConfig, jobConfigData);
        when(buildDecision.getStatus(globalConfigDataForSonarInstance,jobConfigData)).thenReturn(Result.SUCCESS, Result.FAILURE);
        jenkinsRule.assertBuildStatus(Result.FAILURE, buildProject(freeStyleProject));
        Run lastRun = freeStyleProject._getRuns().newestValue();
    ...
    public void testPerformShouldCatchQGException() throws Exception {
        ...
        jobConfigData.setProjectKey("projectKey");
        doReturn(globalConfigDataForSonarInstance).when(buildDecision).chooseSonarInstance(globalConfig, jobConfigData);
        doReturn(Result.SUCCESS).doThrow(exception).when(buildDecision).getStatus(any(GlobalConfigDataForSonarInstance.class),any(JobConfigData.class));
        jenkinsRule.assertBuildStatus(Result.FAILURE, buildProject(freeStyleProject));
        Run lastRun = freeStyleProject._getRuns().newestValue();
